...
function downloadVideo (link, filename, callback) {
  ...
  ytdl.stdout.on('data', (data) => {
    if (data.includes('Duration')) {
      dur = moment(`${today} ${data.split('Duration: ')[1].split(',')[0].split('.')[0]}`).diff(moment(today), 'seconds')
    }
  })
  ytdl.stderr.on('data', (data) => {
    ...
    if (data.includes('time=')) {
      const downloaded = moment(`${today} ${data.split('time=')[1].split(' ')[0].split('.')[0]}`).diff(moment(today), 'seconds')
      const speed = data.split('speed=')[1].split('x')[0]
      const statusPercent = Math.ceil((downloaded / dur) * 100)
      const statusStr = statusPercent === 100 ? '正在完成下載' : `已下載 ${Math.ceil((downloaded / dur) * 100)} % - 大約還有 ${statusTime(Math.ceil((dur - downloaded) / speed))}`
