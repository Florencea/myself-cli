...
async function crawler() {
  ...
  const totalPage = parseInt($($('.last')[0]).text().split(' ')[1], 10);
  ...
  for (let currentPage = 1; currentPage < totalPage; currentPage++) {
    ...
    statusSpinner.text = `已完成 ${Math.ceil((currentPage / totalPage) * 100)} % - 大約還有 ${statusTime(
      Math.ceil(processTimeDeltaAvg * (totalPage - currentPage)),
    )}\n`;
  }
  statusSpinner.stop();
  const bangumiArr = [];
